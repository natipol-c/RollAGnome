--[[
  ...
  Method:   decompile
  Name:     LightningStorm
  Path:     game.StarterPlayer.StarterPlayerScripts.Client.Controllers.GlobalEvents.Events.LightningStorm
...
return {
    OnStart = function(p1, p2) -- Line: 23, Name: OnStart
        ...
        task.spawn(function() -- Line: 24
            if _G.FadeIn then
                _G.FadeIn("LightningStorm", "BG Music", 5);
            end;
        end);
        LightingController:Apply("LightningStorm");
        p1:Add(function() -- Line: 31
            -- upvalues: LightingController (ref)
            LightingController:Remove("LightningStorm");
            task.spawn(function() -- Line: 33
                if _G.FadeIn then
                    _G.FadeIn("BG Music", "LightningStorm", 5);
                end;
            end);
        end);
    end,
